...
describe("settings navigation copy", () => {
  it("uses readable localized labels and shows usage tab", () => {
    ...
    expect(source).toContain("visibleTab === 'diagnostics'")
    expect(source).toContain("DiagnosticsSettingsTab")
    expect(source).toContain("window.electronAPI.diagnostics.recentLogs")
    ...
    const allSources = source
      ...
      + readFileSync(join(process.cwd(), "src/renderer/screens/McpSettingsTab.tsx"), "utf8")
      + readFileSync(join(process.cwd(), "src/renderer/screens/WorkspacesTab.tsx"), "utf8")
    ...
    expect(allSources).toContain("tr('MCP 服务', 'MCP services')")
