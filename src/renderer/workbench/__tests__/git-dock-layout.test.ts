...
describe("workbench git dock layout", () => {
  ...
  it("keeps send-prompt dispatch request routing outside WorkbenchLayout", () => {
    ...
    const sendPromptStart = layout.indexOf("const sendPromptOutcome = async")
    ...
    expect(layout).toContain("createTurnAndRefresh, resolveDispatchRequest")
    expect(sendPromptSource).toContain("const dispatchRequest = resolveDispatchRequest")
    expect(sendPromptSource).toContain("const sendComposerPrompt = async")
    expect(sendPromptSource).toContain("return outcome.ok ? { ok: true } : outcome")
    expect(layout).toContain("sendComposerPrompt={sendComposerPrompt}")
    expect(mainContent).toContain("onSend: sendComposerPrompt")
    expect(mainContent).not.toContain("return result ? { ok: true } : { ok: false, reason: 'send-failed' }")
