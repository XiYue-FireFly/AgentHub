...
describe("requirements document chat dispatch", () => {
  it("exposes a first-class action to send the full requirement document into chat", () => {
    ...
    expect(editor).toContain("按文档开发")
    expect(editor).toContain("Develop from doc")
    ...
    expect(requirements).toContain("window.electronAPI.todos.syncFromMarkdown(threadId, latest.content, source)")
