 ...
 *   - --permission-mode acceptEdits：自动接受文件编辑；Bash 等其它操作仍需确认（非交互管道下
 *     agent 需运行命令时可能挂起等待直至超时，若需全自动可改用 bypassPermissions，但有安全风险）。
 *     其它可选项：default（每次确认）/ plan（只读）/ bypassPermissions（全开无确认）。
