...
import { DEFAULT_SKILL_CATEGORY, SkillDef } from '../types'

/** buildSkillBlock 单测 — 覆盖：空→空串、多技能拼装、字符上限省略计数。 */

function skill(name: string, instructions: string, description = ''): SkillDef {
  return { id: 'id-' + name, name, category: DEFAULT_SKILL_CATEGORY, description, instructions, tags: [], source: 'test', createdAt: 0, updatedAt: 0 }
}
